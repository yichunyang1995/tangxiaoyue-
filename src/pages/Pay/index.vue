    ...
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"><em class="lead">应付金额：</em><em class="orange money">￥{{msg}}</em></span>
        </div>
      </div>
  ...
  export default {
   ...
   async   getData(){
    //  console.log(this.orderId)
     ...
     let result=  await this.$API.reqOrderMsg(this.orderId)
    //  console.log(result)
     this.datas=result.data
     if(result.code==200){
       this.msg=result.data.totalFee
     }else{
       alert:('没有获取到数据')
     }
   },
   async open() {
     ...
     //生成支付二维码
     let url= await QRCode.toDataURL(this.datas.codeUrl)
    // console.log(url)
